...
import type { StoreState, Dispatch, Terminal as TerminalType, Directory } from 'types';
...
class TerminalLink extends React.Component<Props, State> {
    ...
    handleKey = (e: SyntheticKeyboardEvent<HTMLDivElement>) => {
        ...
        if (e.keyCode === Constants.KEY_ENTER) {
            ...
            if (words.length > 1) {
                ...
                // Grab Matches
                const workingDirectory = getDirectory(this.props.terminal.workingDirectory, store.getState().wfs);
                ...
                if (matches.length === 1) {
                    ...
                    const name = matches[0].name.indexOf(' ') >= 0 ? '"' + matches[0].name + '"' : matches[0].name;
                    ...
                    for (let i = 0; i < matches.length; i++) {
                        this.props.addCommand(
                            (workingDirectory.data[i].type === Constants.DIR_TYPE ? 'DIR     ' : 'FILE    ') +
                                workingDirectory.data[i].name,
                            false
                        );
                    }
                } else {
    ...
    render() {
        ...
        return (
            <div className="terminal-link" ref={input => (this.input = input)} tabIndex="1" onKeyDown={this.handleKey}>
                <Terminal
